...
import { getFirestore, collection, query, orderBy, where, limit, getDocs, FirebaseFirestoreTypes } from "@react-native-firebase/firestore";
...
async function fetchTopBusinesses(limitCount: number, minReviews: MinReviews): Promise<AdminBusiness[]> {
...
export default function AdminDashboardScreen() {
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalReviews = filteredBusinesses.reduce(
    ...
    if (totalReviews > 0) {
      weightedRating =
        filteredBusinesses.reduce(
          (sum, b) => sum + (b.avgRating || 0) * (b.reviewCount || 0),
          0
        ) / totalReviews;
    }
  ...
  const newlyActiveBusinesses = useMemo(() => {
    ...
    return withLastReview
      .sort(
        (a, b) => getTimeSafe(b.lastReviewAt) - getTimeSafe(a.lastReviewAt)
      )
      .slice(0, 5);
  }, [filteredBusinesses]);
        ...
        <View style={styles.minReviewsRow}>
          ...
          {[1, 3, 5].map((value) => (
            <TouchableOpacity
              ...
              style={[
                styles.minReviewsChip,
                minReviews === value && styles.minReviewsChipActive,
              ]}
              ...
              newlyActiveBusinesses.map((b) => (
                <View key={b.id} style={styles.cityRow}>
                  ...
                  <Text style={styles.cityMeta}>
                    {formatLastReviewDate(b.lastReviewAt)}
                  </Text>
                </View>
              ))
            )}
            ...
            <View style={styles.cardStats}>
              <View style={styles.chipRow}>
                <View style={styles.statChipPrimary}>
                  <Text style={styles.statChipTextPrimary}>{b.reviewCount} review{b.reviewCount === 1 ? "" : "s"}</Text>
                </View>
